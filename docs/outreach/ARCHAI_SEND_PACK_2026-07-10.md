# ARCHAI OUTREACH — SEND PACK (ready to action)

_Generated 2026-07-09. Contacts verified against the two contact databases + live web research (July 2026). Each entry: correct contact · its template · a ready-to-paste script. Personalise `[Name/Team]` where you can. Log every send in the Send Log tab of the Definitive Contact Database. Any rights/correction reply is actioned first, before partnership talk._

**56 organisations · 30 with a direct email ready now · rest are web-form routes (📝).**

**Tomorrow-morning order:** Group A (source institutions already in ARCHAI — a courtesy/transparency note, highest goodwill), then Group B (Australian & warm pilots — your real targets), then C (technical partners), then D (sector advisors). A(prospective) and forms can be a second pass.

---


# TEMPLATE A · Source institution (already in ARCHAI)  (18)

## 📧 Art Institute of Chicago  —  _High priority_
**Send to:** engineering@artic.edu
**Fit:** Public API and open access engineering contact; good technical feedback path

```
Subject: ARCHAI research prototype using your open collection data

Hi Art Institute of Chicago Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Art Institute of Chicago, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your public API and open-access programme are exactly the kind of well-documented open data ARCHAI is built to use responsibly, and your engineering team's perspective on how it is represented would be genuinely valuable.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Art Institute of Chicago. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Cleveland Museum of Art  —  _High priority_
**Send to:** openaccess@clevelandart.org  ·  info@clevelandart.org
**Fit:** CC0/open access collection; ask for rights/framing feedback and pilot interest

```
Subject: ARCHAI research prototype using your open collection data

Hi Cleveland Museum of Art Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Cleveland Museum of Art, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your CC0 open-access collection is a natural fit for a rights-aware system like this, and I would value your feedback on how the records are framed and whether a small pilot might be of interest.

Two of your own records are already speaking — try the [Red-Figure Fish Plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) or [Van Gogh's portrait of Adeline Ravoux](https://archai-api.fineartmedia.tech/aux/NFC154.html). The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Cleveland Museum of Art. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 National Gallery of Art, Washington  —  _High priority_
**Send to:** ngaimages@nga.gov (image rights)  ·  open-data issues at github.com/NationalGalleryOfArt/opendata
**Fit:** CC0 dataset; strong open data precedent; ask open access team

```
Subject: ARCHAI research prototype using your open collection data

Hi National Gallery of Art Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from National Gallery of Art, Washington, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your CC0 dataset and leadership in open data make you an important reference point for this work, and I would welcome your open-access team's view on how it is represented.

One of your own records is already speaking — [Advance of the Skirmish Line](https://archai-api.fineartmedia.tech/aux/NFC746.html) will answer questions grounded strictly in its record. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by National Gallery of Art, Washington. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Rijksmuseum  —  _High priority_
**Send to:** collectieinfo@rijksmuseum.nl
**Fit:** Major open data image collection; ask data services for rights/framing feedback

```
Subject: ARCHAI research prototype using your open collection data

Hi Rijksmuseum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Rijksmuseum, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your open-data image collection is one of the richest of its kind, and I would value your data services team's feedback on how the records and their rights are handled.

One of your own records is already speaking — the [portrait of Floris V](https://archai-api.fineartmedia.tech/aux/NFC1000.html) will answer questions grounded strictly in its record. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Rijksmuseum. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Smithsonian Institution  —  _High priority_
**Send to:** openaccess@si.edu
**Fit:** Huge CC0 dataset; strong sovereign/local AI demo relevance

```
Subject: ARCHAI research prototype using your open collection data

Hi Smithsonian Institution Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Smithsonian Institution, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your large CC0 dataset is an ideal way to show that meaningful, sovereign, local AI can work at real scale on openly-licensed cultural data.

One of your own records is already speaking — your [Beechcraft King Air 65-90](https://archai-api.fineartmedia.tech/aux/NFC1230.html) will answer questions grounded strictly in its record. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Smithsonian Institution. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Te Papa Tongarewa  —  _High priority_
**Send to:** collections.api@tepapa.govt.nz  ·  digitaloutreach@tepapa.govt.nz
**Fit:** API terms include caching limits and cultural care; important to ask about research cache/refresh cycle

```
Subject: ARCHAI research prototype using your open collection data

Hi Te Papa Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Te Papa Tongarewa, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your API's attention to caching and cultural care matters a great deal to how ARCHAI should behave, and I would like to check I am handling the research cache and refresh cycle in line with your terms.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Te Papa Tongarewa. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 The Metropolitan Museum of Art  —  _High priority_
**Send to:** openaccess@metmuseum.org  ·  image.licensing@metmuseum.org
**Fit:** CC0 open data; strong open access precedent; ask for representation/rights feedback and possible research conversation

```
Subject: ARCHAI research prototype using your open collection data

Hi Metropolitan Museum of Art Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from The Metropolitan Museum of Art, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your CC0 open-access programme set much of the precedent this work builds on, and I would welcome your feedback on representation and rights, and any interest in a research conversation.

One of your own records is already speaking — your [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html), the oldest surviving piano in the world, will answer questions grounded strictly in its record. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by The Metropolitan Museum of Art. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Victoria and Albert Museum  —  _High priority_
**Send to:** hello@vam.ac.uk  ·  developers.vam.ac.uk (API docs)
**Fit:** Strong IIIF/API fit; ask digital collections team for feedback

```
Subject: ARCHAI research prototype using your open collection data

Hi Victoria and Albert Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Victoria and Albert Museum, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your IIIF and API infrastructure is a strong technical fit for ARCHAI, and I would value your digital collections team's feedback on how it is used.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Victoria and Albert Museum. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Wellcome Collection  —  _High priority_
**Send to:** collections@wellcomecollection.org  ·  info@wellcomecollection.org
**Fit:** API + IIIF + sensitive health/care collections; good governance conversation

```
Subject: ARCHAI research prototype using your open collection data

Hi Wellcome Collection Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Wellcome Collection, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your API and IIIF access, alongside sensitive health and care material, make you an especially valuable partner for a conversation about responsible governance.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Wellcome Collection. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Auckland War Memorial Museum  —  _Medium priority_
**Send to:** CollectionEnquiries@aucklandmuseum.com  ·  bjhoare@aucklandmuseum.com (Brodie Hoare, API/technical)
**Fit:** API source; ask about approved derivative image paths and placeholder handling

```
Subject: ARCHAI research prototype using your open collection data

Hi Auckland War Memorial Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Auckland War Memorial Museum, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your API is already a source in the prototype, and I would like to check I am using approved derivative image paths and handling placeholders the way your team would want.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Auckland War Memorial Museum. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Cooper Hewitt Smithsonian Design Museum  —  _Medium priority_
**Send to:** web form / route: General/open data route
**Fit:** Design collection + Smithsonian ecosystem; strong design metadata fit

```
Subject: ARCHAI research prototype using your open collection data

Hi Cooper Hewitt Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Cooper Hewitt Smithsonian Design Museum, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your design collection and rich design metadata, within the Smithsonian ecosystem, are a strong fit for how ARCHAI interprets objects.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Cooper Hewitt Smithsonian Design Museum. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Europeana  —  _Medium priority_
**Send to:** api@europeana.eu  ·  info@europeana.eu
**Fit:** Aggregator rights complexity and multilingual access; ask data services/reuse team

```
Subject: ARCHAI research prototype using your open collection data

Hi Europeana Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Europeana, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: as a multilingual aggregator with genuinely complex rights, you are an important test of how ARCHAI handles both, and I would value your data services and reuse team's perspective.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Europeana. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Instituto Brasileiro de Museus / Brasiliana Museus  —  _Medium priority_
**Send to:** cgsim@museus.gov.br
**Fit:** South American open collection perspective; bilingual outreach

```
Subject: ARCHAI research prototype using your open collection data

Hi Brasiliana Museus Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Instituto Brasileiro de Museus / Brasiliana Museus, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your open collection brings an essential South American perspective to this work, and I would be glad to continue in Portuguese or English, whichever suits your team.

One of your own records is already speaking — an [estampa religiosa](https://archai-api.fineartmedia.tech/aux/NFC120.html) from Brasiliana will answer questions grounded strictly in its record. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Instituto Brasileiro de Museus / Brasiliana Museus. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 J. Paul Getty Museum / Getty  —  _Medium priority_
**Send to:** reference@getty.edu
**Fit:** Open content + vocabularies/AAT; good standards partner

```
Subject: ARCHAI research prototype using your open collection data

Hi Getty Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from J. Paul Getty Museum / Getty, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your Open Content programme and vocabularies such as the AAT make you a natural standards partner for a project built on open, interoperable data.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by J. Paul Getty Museum / Getty. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Library of Congress  —  _Medium priority_
**Send to:** LC-Labs@loc.gov
**Fit:** Open APIs, national library scale, public cultural access

```
Subject: ARCHAI research prototype using your open collection data

Hi Library of Congress Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Library of Congress, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your open APIs and national-library scale make you a landmark example of public cultural access, exactly the kind of openness ARCHAI is designed to serve.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Library of Congress. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 M+, Hong Kong  —  _Medium priority_
**Send to:** stories@mplus.org.hk (collections)  ·  register for API key at dashboard.mplus.org.hk
**Fit:** Bilingual contemporary visual culture; ask about preview-media handling

```
Subject: ARCHAI research prototype using your open collection data

Hi M+ Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from M+, Hong Kong, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your bilingual, contemporary visual culture is a strong fit for ARCHAI's multilingual interpretation, and I would like to check I am handling preview media appropriately.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by M+, Hong Kong. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 RAWG Video Games Database  —  _Medium priority_
**Send to:** web form / route: API/support route
**Fit:** Video games metadata; ask attribution/linking check

```
Subject: ARCHAI research prototype using your open collection data

Hi RAWG Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from RAWG Video Games Database, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your video-games metadata lets ARCHAI treat born-digital works as first-class cultural objects, and I would like to confirm I am attributing and linking back to you correctly.

One of the records is already speaking — [Tetris (1984)](https://archai-api.fineartmedia.tech/aux/NFC883.html), talking as a first-class cultural object. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by RAWG Video Games Database. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Tate  —  _Medium priority_
**Send to:** web form / route: GitHub repository / Tate contact route
**Fit:** Open metadata; ask about CC BY attribution and daily-changing metadata

```
Subject: ARCHAI research prototype using your open collection data

Hi Tate Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm writing because ARCHAI currently includes a carefully selected subset of records from Tate, using your published open collection data / API and preserving attribution, source links and item-level rights. Before the project grows, I wanted to introduce myself, explain what I'm building, and make sure the work represents your collection appropriately.

ARCHAI is an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages. The preferred deployment keeps collection data, vector search and AI inference under institutional control.

Relevance to your collection: your open metadata is a strong fit for this work, and I would like to check I am handling CC BY attribution and your regularly-updated records the way your team expects.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Use of records in the demo does not imply endorsement by Tate. If anything is incorrect, mislabelled, culturally sensitive, or unsuitable, I will correct, restrict or remove it promptly.

Would someone in your digital collections, open access, API, rights or research team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```


# TEMPLATE A(prospective) · Open collection we would like to include  (18)

## 📝 National Library of Australia / Trove  —  _High priority_
**Send to:** web form / route: Trove support/contact route
**Fit:** National discovery service; Trove API; strong federation/data governance relevance

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi National Library of Australia Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

National Library of Australia / Trove is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: as a national discovery service, Trove is a natural fit for ARCHAI's federated, sovereignty-first approach to data governance.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 British Library  —  _Medium priority_
**Send to:** web form / route: Contact route
**Fit:** Digital collections, metadata, preservation; library-sector fit

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi British Library Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

British Library is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your digital collections, metadata depth and preservation focus make you a strong fit for how ARCHAI serves the library sector.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 British Museum  —  _Medium priority_
**Send to:** web form / route: Contact route
**Fit:** Major world collection; sensitive governance/protocol care required

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi British Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

British Museum is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your collection's global significance, and the governance and protocol care it rightly demands, is exactly the kind of context ARCHAI is designed to respect.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Brooklyn Museum  —  _Medium priority_
**Send to:** web form / route: General/open data route
**Fit:** Transparent rights fields/open collection model

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Brooklyn Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Brooklyn Museum is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your transparent rights fields and open-collection model are a natural match for a rights-aware system like ARCHAI.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Brussels Open Data  —  _Medium priority_
**Send to:** web form / route: Open data portal contact
**Fit:** Street-art/public culture dataset; CC BY 4.0 attribution check

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Brussels Open Data Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Brussels Open Data is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your street-art and public-culture data extends ARCHAI beyond museum walls, and I would want to confirm I am handling CC BY 4.0 attribution correctly.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 DigitalNZ  —  _Medium priority_
**Send to:** web form / route: Contact route
**Fit:** Aotearoa aggregation; partner rights gates; multilingual/regional breadth

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi DigitalNZ Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

DigitalNZ is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your Aotearoa aggregation, partner rights gates and regional breadth make you a strong fit for ARCHAI's multilingual, rights-respecting approach.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Harvard Art Museums  —  _Medium priority_
**Send to:** web form / route: General/contact route
**Fit:** Structured API and strong metadata; data quality conversation

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Harvard Art Museums Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Harvard Art Museums is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your well-structured API and strong metadata would make you an excellent partner for a conversation about data quality in AI interpretation.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 MoMA  —  _Medium priority_
**Send to:** web form / route: General contact / GitHub open data
**Fit:** Modern/contemporary art, media art relevance

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi MoMA Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

MoMA is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your modern, contemporary and media-art holdings are directly relevant to ARCHAI's work with born-digital and time-based objects.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 National Gallery, London  —  _Medium priority_
**Send to:** web form / route: API documentation/contact route
**Fit:** Interoperable data and IIIF/images; strong standards case

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi National Gallery, London Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

National Gallery, London is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your interoperable data and IIIF imagery make a strong standards case for how ARCHAI should handle openly-published collections.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Science Museum Group  —  _Medium priority_
**Send to:** web form / route: Contact route
**Fit:** Science/technology/media/transport collection gap filler

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Science Museum Group Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Science Museum Group is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your science, technology, media and transport holdings fill a genuine gap in the kinds of objects ARCHAI can currently interpret.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Canadian Museum of History  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** National cultural history; future pilot/research target

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Canadian Museum of History Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Canadian Museum of History is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your national cultural-history collection would be a compelling future pilot and research partner for this work.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 LACMA  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** Large art collection; possible future collection interface test

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi LACMA Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

LACMA is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your large art collection would be an ideal future test of ARCHAI as a collection-scale interpretive interface.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Museo Nacional de Bellas Artes  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** South American representation beyond Brasiliana

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Museo Nacional de Bellas Artes Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Museo Nacional de Bellas Artes is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your collection would broaden ARCHAI's South American representation beyond its current Brazilian source.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 National Gallery of Canada  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** National art collection; future comparative open collection candidate

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi National Gallery of Canada Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

National Gallery of Canada is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your national art collection would be a strong future candidate for comparative work across open collections.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 National Museum of Korea  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** Major Korean collection; future multilingual access case

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi National Museum of Korea Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

National Museum of Korea is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your collection would be a significant future case for ARCHAI's multilingual access, opening Korean heritage to more visitors in their own language.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 National Palace Museum  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** Major Asian collection; multilingual/cross-cultural interpretation relevance

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi National Palace Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

National Palace Museum is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your collection is directly relevant to ARCHAI's multilingual, cross-cultural interpretation of major Asian holdings.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Royal Ontario Museum  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** Natural/cultural history breadth; future pilot target

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Royal Ontario Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Royal Ontario Museum is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your breadth across natural and cultural history would make you a strong future pilot for ARCHAI.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Tokyo National Museum  —  _Low priority_
**Send to:** web form / route: Contact route
**Fit:** Major Asian collection; multilingual interpretation future case

```
Subject: ARCHAI — a research prototype that could include your open collection

Hi Tokyo National Museum Open Access / Digital Collections team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first research prototype for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and supports rights-aware semantic search, staff-facing collection interrogation, and AUXIO object-level visitor pages, with collection data and AI kept under institutional control.

Tokyo National Museum is a strong candidate to include next, using only your published open/CC data and preserving attribution, source links and item-level rights. Before harvesting anything, I wanted to introduce myself and check you'd be comfortable with a small, clearly-labelled research subset — and correct, restrict or remove anything on request.

Why your collection: your collection would be an important future case for ARCHAI's multilingual interpretation of major Asian heritage.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Would someone in your open access, API or digital collections team be open to a short conversation?

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```


# TEMPLATE B · Australian pilot / warm institution  (11)

## 📧 ACMI  —  _High priority_
**Send to:** collections.access@acmi.net.au
**Fit:** Born-digital, screen culture, time-based media; strong ARCHAI preservation/replay fit

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi ACMI team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought ACMI could be a strong fit because your born-digital, screen-culture and time-based collections are exactly where ARCHAI's preservation and replay work is most useful.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Heide Museum of Modern Art  —  _High priority_
**Send to:** collections@heide.com.au  ·  info@heide.com.au
**Fit:** Outdoor/sculpture/garden QR/NFC pilot; warm prior work context

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi Heide Museum of Modern Art team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought Heide Museum of Modern Art could be a strong fit because your outdoor sculpture and garden setting is a natural home for a QR/NFC wayfinding and interpretation pilot, and we have had warm contact before.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Museums Victoria  —  _High priority_
**Send to:** AskUs@museum.vic.gov.au  ·  research-enquiries form via museumsvictoria.com.au/ask-us
**Fit:** Founding/demo collection; local relationship; best pilot prospect

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi Museums Victoria team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought Museums Victoria could be a strong fit because your collection has been part of ARCHAI's development from the start, you are local to me, and I think you would make an ideal first pilot.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 NFSA  —  _High priority_
**Send to:** access@nfsa.gov.au  ·  enquiries@nfsa.gov.au
**Fit:** Audiovisual, games, new media, preservation; strong ARCHAI direction

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi NFSA team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought NFSA could be a strong fit because your audiovisual, games and new-media holdings, and your preservation focus, sit right at the centre of where ARCHAI is heading.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 National Museum of Australia  —  _High priority_
**Send to:** api@nma.gov.au  ·  information@nma.gov.au
**Fit:** Collection API with public domain/CC images; national history depth

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi National Museum of Australia team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought National Museum of Australia could be a strong fit because your collection API with public-domain and CC imagery, and your depth in national history, are a strong match for ARCHAI.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 Powerhouse Museum / MAAS  —  _High priority_
**Send to:** api@maas.museum  ·  info@powerhouse.com.au
**Fit:** Technology-rich collection; CC BY/API candidate; strong applied arts/science fit

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi Powerhouse Museum team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought Powerhouse Museum / MAAS could be a strong fit because your technology-rich collection and your applied-arts and science focus are a strong fit for ARCHAI, and your CC BY / API access makes it practical.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 QAGOMA  —  _High priority_
**Send to:** gallery@qagoma.qld.gov.au
**Fit:** Asia-Pacific collection strength; ask about approved image path and possible pilot

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi QAGOMA team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought QAGOMA could be a strong fit because your Asia-Pacific collection strength is a compelling fit for ARCHAI, and I would love to talk about an approved image path and a possible pilot.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 State Library Victoria  —  _High priority_
**Send to:** accessinclusion@slv.vic.gov.au  ·  Ask a Librarian form
**Fit:** Digitised collections, oral histories, public access; strong library-sector test

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi State Library Victoria team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought State Library Victoria could be a strong fit because your digitised collections, oral histories and public-access mission would make a strong library-sector test for ARCHAI.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 TarraWarra Museum of Art  —  _High priority_
**Send to:** museum@twma.com.au
**Fit:** Regional gallery AUX.IO interpretation pilot; warm prior relationship

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi TarraWarra Museum of Art team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought TarraWarra Museum of Art could be a strong fit because your regional gallery would be an ideal setting for an AUXIO interpretation pilot, and we have had a warm relationship already.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 National Gallery of Australia  —  _Medium priority_
**Send to:** information@nga.gov.au  ·  australian.art@nga.gov.au
**Fit:** flagship national gallery; interpretation/visitor-experience framing and open-collection conversation — credibility for the Australian coalition.

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi National Gallery of Australia team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought National Gallery of Australia could be a strong fit because as a flagship national gallery, your work on interpretation and visitor experience, and an open-collection conversation, would mean a great deal to this project.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 National Gallery of Victoria  —  _Medium priority_
**Send to:** ngvenquiries@ngv.vic.gov.au
**Fit:** Visitor interpretation, access, exhibition tech; flagship credibility

```
Subject: ARCHAI — sovereign collection AI research pilot conversation

Hi National Gallery of Victoria team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit for museums, galleries, libraries and archives. It sits above existing CMS/DAMS systems and adds rights-aware semantic search, staff-facing collection intelligence, and AUXIO conversational visitor pages. The core idea is simple: collection data and visitor interaction stay under the institution's control.

I thought National Gallery of Victoria could be a strong fit because your leadership in visitor interpretation, access and exhibition technology makes you an ideal flagship partner for ARCHAI.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

A working demonstration is already running across open collection records, and a pilot could be scoped to a small, low-risk set of approved records — with rights, terminology, cultural protocols and publication rules set by your team.

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This forms part of my RMIT Design PhD. I'm not asking for a funding commitment at this stage — the first step would simply be a short conversation or demo to see whether the project aligns with your team.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```


# TEMPLATE C · Technical partner  (5)

## 📧 CollectionSpace / LYRASIS  —  _High priority_
**Send to:** collectionspace@lyrasis.org
**Fit:** Open-source collections management; potential adapter and research partner

```
Subject: Research partnership invitation — open CMS/DAM integration for ARCHAI

Hi CollectionSpace team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first semantic interface layer for museums and galleries. ARCHAI is not intended to replace CollectionSpace. It's designed to sit above systems like CollectionSpace, read approved metadata and derivatives, preserve source identifiers and rights, and add local semantic search, governed collection Q&A, multilingual access, and AUXIO object-level visitor pages.

Why CollectionSpace: Open-source collections management; potential adapter and research partner

I'd value a short technical conversation about the cleanest integration pattern — field mapping, incremental sync, rights and derivative-media handling — and whether an open connector could be useful to your community.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This isn't a request for a financial commitment — I'm first trying to identify shared technical questions and possible in-kind research collaboration.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 CollectiveAccess  —  _High priority_
**Send to:** info@collectiveaccess.org  ·  consulting@collectiveaccess.org
**Fit:** Open-source CMS; ARCHAI adapter/federated semantic layer fit

```
Subject: Research partnership invitation — open CMS/DAM integration for ARCHAI

Hi CollectiveAccess team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first semantic interface layer for museums and galleries. ARCHAI is not intended to replace CollectiveAccess. It's designed to sit above systems like CollectiveAccess, read approved metadata and derivatives, preserve source identifiers and rights, and add local semantic search, governed collection Q&A, multilingual access, and AUXIO object-level visitor pages.

Why CollectiveAccess: Open-source CMS; ARCHAI adapter/federated semantic layer fit

I'd value a short technical conversation about the cleanest integration pattern — field mapping, incremental sync, rights and derivative-media handling — and whether an open connector could be useful to your community.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This isn't a request for a financial commitment — I'm first trying to identify shared technical questions and possible in-kind research collaboration.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 IIIF Consortium  —  _Medium priority_
**Send to:** web form / route: IIIF community contact route
**Fit:** Image interoperability, deep zoom, provenance, media standards

```
Subject: Research partnership invitation — open CMS/DAM integration for ARCHAI

Hi IIIF Consortium team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first semantic interface layer for museums and galleries. ARCHAI is not intended to replace IIIF Consortium. It's designed to sit above systems like IIIF Consortium, read approved metadata and derivatives, preserve source identifiers and rights, and add local semantic search, governed collection Q&A, multilingual access, and AUXIO object-level visitor pages.

Why IIIF Consortium: Image interoperability, deep zoom, provenance, media standards

I'd value a short technical conversation about the cleanest integration pattern — field mapping, incremental sync, rights and derivative-media handling — and whether an open connector could be useful to your community.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This isn't a request for a financial commitment — I'm first trying to identify shared technical questions and possible in-kind research collaboration.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Omeka / Digital Scholar  —  _Medium priority_
**Send to:** web form / route: Official contact form
**Fit:** Omeka S semantic/conversational publishing layer fit

```
Subject: Research partnership invitation — open CMS/DAM integration for ARCHAI

Hi Omeka team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first semantic interface layer for museums and galleries. ARCHAI is not intended to replace Omeka. It's designed to sit above systems like Omeka, read approved metadata and derivatives, preserve source identifiers and rights, and add local semantic search, governed collection Q&A, multilingual access, and AUXIO object-level visitor pages.

Why Omeka: Omeka S semantic/conversational publishing layer fit

I'd value a short technical conversation about the cleanest integration pattern — field mapping, incremental sync, rights and derivative-media handling — and whether an open connector could be useful to your community.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This isn't a request for a financial commitment — I'm first trying to identify shared technical questions and possible in-kind research collaboration.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 ResourceSpace  —  _Medium priority_
**Send to:** web form / route: Official contact form
**Fit:** Open DAMS; adapter for approved derivatives/permissions

```
Subject: Research partnership invitation — open CMS/DAM integration for ARCHAI

Hi ResourceSpace team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first semantic interface layer for museums and galleries. ARCHAI is not intended to replace ResourceSpace. It's designed to sit above systems like ResourceSpace, read approved metadata and derivatives, preserve source identifiers and rights, and add local semantic search, governed collection Q&A, multilingual access, and AUXIO object-level visitor pages.

Why ResourceSpace: Open DAMS; adapter for approved derivatives/permissions

I'd value a short technical conversation about the cleanest integration pattern — field mapping, incremental sync, rights and derivative-media handling — and whether an open connector could be useful to your community.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

This isn't a request for a financial commitment — I'm first trying to identify shared technical questions and possible in-kind research collaboration.

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```


# TEMPLATE D · Strategic advisor / sector intro  (4)

## 📝 AIATSIS  —  _High priority_
**Send to:** web form / route: Contact route
**Fit:** Indigenous data sovereignty/protocol guidance; advice before sensitive collections

```
Subject: ARCHAI — a short conversation about GLAM research partnerships

Hi AIATSIS team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit that gives museums, galleries, libraries and archives rights-aware semantic search and conversational visitor access while keeping all data and AI under institutional control. It's part of my RMIT Design PhD.

I'm reaching out to AIATSIS for guidance rather than a pilot: Indigenous data sovereignty/protocol guidance; advice before sensitive collections

Would you be open to a short conversation? I'd value your read on sector fit, cultural-protocol safeguards, and who else it would be sensible to talk to.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 AMaGA National  —  _High priority_
**Send to:** info@amaga.org.au
**Fit:** Best path to regional/small museums and sector legitimacy

```
Subject: ARCHAI — a short conversation about GLAM research partnerships

Hi AMaGA National team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit that gives museums, galleries, libraries and archives rights-aware semantic search and conversational visitor access while keeping all data and AI under institutional control. It's part of my RMIT Design PhD.

I'm reaching out to AMaGA National for guidance rather than a pilot: Best path to regional/small museums and sector legitimacy

Would you be open to a short conversation? I'd value your read on sector fit, cultural-protocol safeguards, and who else it would be sensible to talk to.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📧 AMaGA Victoria  —  _High priority_
**Send to:** enquiries@amagavic.org.au
**Fit:** Local Victorian network; good regional pilot pathway

```
Subject: ARCHAI — a short conversation about GLAM research partnerships

Hi AMaGA Victoria team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit that gives museums, galleries, libraries and archives rights-aware semantic search and conversational visitor access while keeping all data and AI under institutional control. It's part of my RMIT Design PhD.

I'm reaching out to AMaGA Victoria for guidance rather than a pilot: Local Victorian network; good regional pilot pathway

Would you be open to a short conversation? I'd value your read on sector fit, cultural-protocol safeguards, and who else it would be sensible to talk to.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```

## 📝 Local Contexts  —  _High priority_
**Send to:** web form / route: Contact route
**Fit:** TK Labels and cultural authority protocols; vital before sensitive collections

```
Subject: ARCHAI — a short conversation about GLAM research partnerships

Hi Local Contexts team,

I hope you're well. My name is Rob Graham — a Melbourne-based creative technologist, founder of FAMTEC, and a PhD researcher in Design at RMIT University.

I'm developing ARCHAI, an open-source, local-first toolkit that gives museums, galleries, libraries and archives rights-aware semantic search and conversational visitor access while keeping all data and AI under institutional control. It's part of my RMIT Design PhD.

I'm reaching out to Local Contexts for guidance rather than a pilot: TK Labels and cultural authority protocols; vital before sensitive collections

Would you be open to a short conversation? I'd value your read on sector fit, cultural-protocol safeguards, and who else it would be sensible to talk to.

You can meet a couple of the objects already speaking — the Met's [1720 Cristofori grand piano](https://archai-api.fineartmedia.tech/aux/NFC495.html) and a [2,300-year-old Greek fish plate](https://archai-api.fineartmedia.tech/aux/NFC198.html) from Cleveland. The thinking behind the project is in my essay, [Waking Up the Museum](https://open.substack.com/pub/robgraham1/p/a-manifesto-for-ethical-community?r=3nsjjn).

Project: [fineartmedia.tech/archai](https://fineartmedia.tech/archai)
GitHub: [github.com/rob-e-graham/archai](https://github.com/rob-e-graham/archai)

Warm regards,

Rob Graham
Founder, FAMTEC
PhD Researcher, School of Design, RMIT University
rob@fineartmedia.tech · [fineartmedia.tech](https://fineartmedia.tech)
LinkedIn: [Rob Graham](https://au.linkedin.com/in/rob-graham-a0897a25)
```