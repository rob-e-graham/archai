# ARCHAI Partnership Email — Curatorial Schools & Art Education

**Track**: University curatorial programmes, museum studies, art history departments, fine art schools
**Hook**: ARCHAI as a live pedagogical tool — real collections, real conversational AI, real-world deployment
**Call to action**: Pilot ARCHAI in a class / supervise student projects / use posters in studio projects

---

## Subject lines (A/B options)

**Option A** (pedagogical, direct):
> A live museum AI for your curatorial students — free, open source, deployable

**Option B** (student project angle):
> Give your students a real collection to work with — ARCHAI in the curriculum

**Option C** (research angle):
> Open-source infrastructure for teaching digital curatorship and AI-mediated collection access

---

## Email body

---

Dear [Prof / Dr / Name],

I'm Rob, co-founder of ARCHAI — an open-source AI platform that makes museum collections conversational and physically deployable via NFC tags and QR codes.

I'm reaching out because we think ARCHAI could be a genuinely useful live tool for curatorial and museum studies programmes — and I'd love to explore a pilot with your department.

**What ARCHAI does:**

ARCHAI connects students to real collections — The Met, Rijksmuseum, Tate, Wellcome Collection, Smithsonian, Getty, Te Papa, QAGOMA, and twelve more — through a conversational AI layer built on local language models. Students can:

- Browse 20,000+ objects across 17 international institutions
- Have natural conversations with objects: ask about provenance, technique, context, conservation
- Generate visitor pages instantly — scannable via NFC or QR, deployable anywhere
- Download high-resolution posters (A4 / A2 / A0 / sticker / postcard) for physical interventions
- Study the full ethical and legal framework underpinning open-access museum data

**Pedagogical applications:**

- *Digital Curatorship*: Students build and deploy their own thematic collections using ARCHAI's Qdrant + Ollama infrastructure
- *Collection Access & Legal Frameworks*: Our legal harvest-bot and collection-targets matrix give students a real worked example of CC0, CC BY, OGL, and metadata-only constraints
- *Physical Exhibition Design*: The NFC/QR poster system lets students design real-world scanning experiences — for hallways, libraries, cafés, public spaces
- *AI Ethics in Cultural Heritage*: Local LLM (no data sent to third parties), open source, no vendor lock-in — good model for discussion
- *Street Art & Interventions*: Our open call invites students to take ARCHAI posters into public space (#ARCHAIinTheWild)

**What we're offering:**

- Free hosted pilot for one class/cohort (or self-hosted — full MIT licence)
- Guest lecture / workshop on AI infrastructure for cultural heritage (remote or in-person)
- Student project mentorship — we're happy to supervise ARCHAI-adjacent thesis or studio projects
- Co-authorship on any resulting research or case study, if that's useful

We're a small team building this for the long term, not as a startup exit. We care about it ending up in classrooms and institutions, not just products.

Would a 20-minute call make sense to explore what might work for your programme?

Warm regards,
Rob Graham
ARCHAI — rob@fineartmedia.tech
fineartmedia.tech/archai / GitHub: rob-e-graham/archai

---

## Follow-up (10 days, no reply)

**Subject**: Quick follow-up — ARCHAI for curatorial students

Hi [Name],

Checking in on my note about ARCHAI as a teaching tool for curatorial programmes.

The short version: it's a free, open-source museum AI — 20,000+ real objects, conversational access, NFC/QR deployment, downloadable posters. Students can deploy their own collection pages within a session.

Happy to share a 5-minute demo video or talk through what a class pilot might look like whenever suits.

Rob

---

## Key messaging pillars for this track

1. **Real infrastructure, not a toy** — actual museum APIs, real Qdrant/Ollama stack, real legal frameworks
2. **Hands-on and deployable** — students produce real artefacts (NFC pages, posters, exhibitions) not just essays
3. **Pedagogically layered** — covers AI ethics, legal frameworks, collection access, exhibition design, physical deployment
4. **Open source, no cost** — MIT licence, self-hostable, no vendor dependency
5. **Research-friendly** — co-authorship, thesis mentorship, case study opportunities

## Institutions to target first

**UK & Ireland**:
- Royal College of Art (Contemporary Art Curating MA)
- Goldsmiths (MA Curating)
- Chelsea College of Arts (MA Curating & Collections)
- University of Glasgow (Museum Studies)
- University College Dublin (Arts Management)

**Australia & NZ**:
- RMIT (Art in Public Space, Museum Studies)
- University of Melbourne (Cultural Heritage Management)
- AUT Auckland (Creative Technologies)
- Queensland College of Art (Griffith University)

**North America**:
- Bard Center for Curatorial Studies
- CalArts
- RISD (Museum Studies)
- School of the Art Institute of Chicago

**Europe**:
- De Appel (Amsterdam)
- École du Louvre
- Städelschule (Frankfurt)
- ZHdK (Zurich)

## Tone notes

- Respectful of academic culture — lead with the pedagogical value, not startup energy
- Offer co-authorship and mentorship early — academics respond to research framing
- Keep the technical depth visible — this is not a no-code tool, it's real infrastructure
- Reference the legal/ethical framework — important for arts education credibility
