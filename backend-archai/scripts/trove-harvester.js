...
function resolveApiKey() {
  // Accept the classic name or the Keytec 'archai' project-key name (TROVE_PROJECT_KEY),
  // from the process env first, then the macOS Keychain (service=famtec).
  const names = ['TROVE_API_KEY', 'TROVE_PROJECT_KEY'];
  for (const n of names) if (process.env[n]) return process.env[n];
  for (const n of names) {
    try {
      const k = execSync(`security find-generic-password -s famtec -a ${n} -w`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
      if (k) { console.log(`  ✓ Trove key loaded from Keychain (KeyTec · ${n})`); return k; }
    } catch (_) {}
  }
  return '';
}
...
if (!API_KEY) {
  ...
  console.error('  → Then: famtec add trove   (or)  TROVE_PROJECT_KEY=xxx node trove-harvester.js\n');
  process.exit(1);
}
...
const SEARCH_QUERIES = SINGLE_QUERY ? [SINGLE_QUERY] : [
  ...
  'aboriginal art', 'gold rush', 'botanical illustration', 'natural history specimen',
  'street scene', 'harbour wharf', 'church cathedral', 'homestead farm', 'tram transport',
  'flowers garden', 'wildlife animals', 'sport athletics', 'war memorial soldier',
  'children school', 'bridge river', 'lighthouse coast', 'market shop',
];
...
// Trove partner hosts that do not hotlink cross-origin (verified in-browser) — skip so the demo has no blanks.
const BLOCK_IMG_HOSTS = /(digitallant\.nt\.gov\.au|archives\.cityofsydney\.nsw\.gov\.au)/i;
function mapWork(w) {
  let thumb = pickThumb(w.identifier);
  // require a real image URL with a path (some records return a bare host = broken image)
  if (!thumb || !/^https?:\/\/[^/]+\/[^/].*/.test(thumb)) return null;
  thumb = thumb.replace(/^http:\/\//i, 'https://');   // avoid mixed-content blocks on the HTTPS demo
  if (BLOCK_IMG_HOSTS.test(thumb)) return null;        // hosts that refuse cross-origin embedding
